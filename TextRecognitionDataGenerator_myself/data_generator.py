...
class FakeTextDataGenerator(object):
    @classmethod
    def generate(cls, index, text, text_name, font, out_dir, height, extension, skewing_angle, random_skew, blur, random_blur, background_type, distorsion_type, distorsion_orientation, is_handwritten, name_format, text_color=-1):
        image = None
        ...
        print("new_text_height", new_text_height)
        ...
        if background_type == 0:
            ...
            if new_text_width != 280:
                new_text_width = 280
            #background = BackgroundGenerator.gaussian_noise(new_text_height + 10, new_text_width + 10)
            background = BackgroundGenerator.gaussian_noise(new_text_height + 10, new_text_width + np.random.randint(50))
            ...
            background = BackgroundGenerator.picture(new_text_height + 10, new_text_width + np.random.randint(50))
        ...
        #####################################
        # if name_format == 0:
        #     image_name = '{}_{}.{}'.format(text.strip(), str(index), extension)
        # elif name_format == 1:
        ...
        # 自己加的
        if name_format == 0:
            image_name = '{}_{}.{}'.format(text_name.strip(), str(index), extension)
        elif name_format == 1:
